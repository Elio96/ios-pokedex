...
extension UIImage {
    ...
    var averageColor: UIColor? {
        return getColors(quality: .lowest)?.background
    }
...
extension UIColor {
    var isBright: Bool {
        // algoritmo per determinare il contrasto dei colori: http://www.w3.org/WAI/ER/WD-AERT/#color-contrast
        guard let components = self.cgColor.components else { return false }
        let r = (components[0] * 299)
        let g = (components[1] * 587)
        let b = (components[2] * 114)
